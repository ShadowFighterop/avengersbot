...
async def gban(userbot):
    ...
    try:
        ...
        return await personalBot.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        ...
        await personalBot.edit(f"`Either reply to a user or gib me user id/name`")
    try:
        if gmute(user.id) is False:
            return await personalBot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await personalBot.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        personalBot = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        personalBot = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await personalBot.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    try:
        ...
        return await personalBot.edit("**Som3ting W3nt Wr0ng**")
    if user:
        if user.id == 1837224931 or user.id == 1761900506 or user.id == 1642269125:
            return await personalBot.edit("**You need to grow some balls to gban / ungban my creator**")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await personalBot.edit(f"Ok! Now Ungbaning this kiddo.\nChats:- `{a}`")
            except:
                b += 1
    else:
        await personalBot.edit("**Reply to a user**")
    try:
        if ungmute(user.id) is False:
            return await personalBot.edit("**Error! User already ungbanned.**")
    except:
        pass
    return await personalBot.edit(
