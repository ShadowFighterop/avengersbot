...
async def _(event):
    ...
    personalevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_spersonal(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    personaluser = await event.client.get_me()
    if personaluser.username:
        curruser = personaluser.username
    else:
        curruser = "personalBot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    personalevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on personalBot server and shows details'
).add_command(
