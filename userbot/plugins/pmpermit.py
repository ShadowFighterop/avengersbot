...
from userbot import ALIVE_NAME, CUSTOM_PMPERMIT, personal_ID
from userbot.Config import Config
from personalBot.utils import admin_cmd
...
USER_BOT_WARN_ZERO = "**You were spamming my sweet master's inbox, henceforth you have been blocked by my master's personalBot.**\n__Now GTFO, i'm busy__"
USER_BOT_NO_WARN = (
    "Hello, This is **personalBot Ultra Private Security Protocol⚠️**.\n"
...
if Var.PRIVATE_GROUP_ID is not None:
        ...
        if len(APPROVED_PMs) > 4095:
            with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
                ...
                await event.client.send_file(
                    ...
                    force_document=True,
                    allow_cache=False,
                    caption="[personalBot]Current Approved PMs",
                    reply_to=event,
                )
                await event.delete()
        else:
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        r = await borg.send_file(
            event.chat_id, personalPIC, caption=USER_BOT_NO_WARN, force_document=False
        )
        PM_WARNS[chat_id] += 1
        if chat_id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_id].delete()
