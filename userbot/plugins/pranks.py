...
"""
# Kang with credits. Using in personalBot...
...
async def personalBot(personalmemes):
    ...
    if not replied:
        await edit_or_reply(
            personalmemes, "`Media file not supported. Reply to a supported media`"
        )
        return
    if replied.media:
        personalmemmes = await edit_or_reply(personalmemes, "`Detecting Threats.........`")
    else:
        await edit_or_reply(
            personalmemes, "`Media file not supported. Reply to a suported media`"
        )
        return
    try:
        ...
        pass
    download_location = await personalmemes.client.download_media(replied, "./temp/")
    if download_location.endswith((".webp")):
        download_location = convert_toimage(download_location)
    size = os.stat(download_location).st_size
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            await personalmemmes.edit(
                "`The replied file is not supported. It should be less than 5mb -_-`"
            )
            os.remove(download_location)
            return
        await personalmemmes.edit("`Detected Threats....`")
    else:
        await personalmemmes.edit("`the replied file is not supported`")
        os.remove(download_location)
        return
    try:
        ...
        await personalmemmes.edit("ERROR: " + str(exc))
        os.remove(download_location)
        return
    personal = f"https://telegra.ph{response[0]}"
    personal = await threats(personal)
    await personalmemmes.delete()
...
async def personalBot(personalmemes):
    ...
    if not replied:
        ...
        return
    if replied.media:
        personalmemmes = await edit_or_reply(personalmemes, "`Detecting Trash....`")
    else:
        await edit_or_reply(
            personalmemes, "`Media file not supported. Reply to a suported media`"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await personalmemmes.edit("`Detected Trash.....`")
    else:
        await personalmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        response = upload_file(download_location)
...
async def personalBot(personalmemes):
    input_str = personalmemes.pattern_match.group(1)
    ...
    if not replied:
        ...
        return
    if replied.media:
        personalmemmes = await edit_or_reply(personalmemes, "`Trapping.....`")
    else:
        await edit_or_reply(
            personalmemes, "Media file not supported. Reply to a suported media"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await personalmemmes.edit("`Trapped...`")
    else:
        await personalmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        ...
        return
    personal = f"https://telegra.ph{response[0]}"
    personal = await trap(text1, text2, personal)
    await personalmemmes.delete()
    await personalmemes.client.send_file(personalmemes.chat_id, personal, reply_to=replied)
...
async def personalBot(personalmemes):
    ...
    if not replied:
        ...
        return
    if replied.media:
        personalmemmes = await edit_or_reply(personalmemes, "`Making A Comment`.")
    else:
        await edit_or_reply(
            personalmemes, "Media file not supported. Reply to a suported media"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await personalmemmes.edit("Commented....")
    else:
        await personalmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        ...
        return
    personal = f"https://telegra.ph{response[0]}"
    personal = await phcomment(personal, text, username)
    await personalmemmes.delete()
    await personalmemes.client.send_file(personalmemes.chat_id, personal, reply_to=replied)
...
CmdHelp("prank").add_command(
  ...
  "prank", None, "If this module doesn't work then contact admins in @personalSupport"
).add()