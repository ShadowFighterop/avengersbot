...
personalBot = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"AB_{user.username}_personalBot_{pack}"
        packnick = (
            f"{personalBot} Vol.{pack}"
            if personalBot
            else f"@{user.username}'s personalBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
