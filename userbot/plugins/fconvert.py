...
from personalBot.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(personal):
    if personal.fwd_from:
        return
    reply_to_id = personal.message.id
    if personal.reply_to_msg_id:
        reply_to_id = personal.reply_to_msg_id
    event = await edit_or_reply(personal, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await personal.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await personal.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    personalreply = await event.get_reply_message()
    if not personalreply or not personalreply.media or not personalreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if personalreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    personalevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, personalreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await personalevent.edit("`This file is not supported`")
            personalresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            personalfile = Path(await event.client.download_media(personalresponse, "./temp/"))
            personalgif = Path(await unzip(personalfile))
            ...
            for files in (personalgif, personalfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await personalevent.edit("Unblock @tgstogifbot")
            return
