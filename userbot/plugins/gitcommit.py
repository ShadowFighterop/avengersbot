...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of personalBot`")
        return
    personalBot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await personalBot.edit(str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await personalBot.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await personalBot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, personalBot)


async def git_commit(file_name, personalBot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await personalBot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await personalBot.edit("Cannot Upload Plugin")
    else:
        return await personalBot.edit("`Committed Suicide`")
