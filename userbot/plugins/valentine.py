...
from personalBot import CmdHelp, bot as personalBot, ALIVE_NAME
from personalBot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Your Lover"

aura = personalBot.uid

personal = f"[{DEFAULTUSER}](tg://user?id={aura})"

@personalBot.on(admin_cmd(pattern="hvd$"))
@personalBot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@personalBot.on(admin_cmd(pattern="vday$"))
@personalBot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
