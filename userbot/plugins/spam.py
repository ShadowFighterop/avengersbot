...
async def bigspam(personal):
    if not personal.text[0].isalpha() and personal.text[0] not in ("/", "#", "@", "!"):
        personal_msg = personal.text
        personalBot_count = int(personal_msg[9:13])
        personal_spam = str(personal.text[13:])
        for i in range(1, personalBot_count):
            await personal.respond(personal_spam)
        await personal.delete()
        if LOGGER:
            await personal.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what personalBot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
