...
async def promote(promt):
    ...
    personalevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await personalevent.edit("Promoted Successfully!")
    except BadRequestError:
        await personalevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    personalevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        await personalevent.edit(NO_PERM)
        return
    await personalevent.edit("Demoted Successfully!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    personalevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await personalevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await personalevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await personalevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    personalevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await personalevent.edit("Unbanned Successfully!")
        if BOTLOG:
        ...
        await personalevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    personalevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await personalevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        ...
        await personalevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
