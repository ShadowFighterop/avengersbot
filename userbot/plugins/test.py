...
async def _(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        ...
        await edit_or_reply(event, "`Testing personalBot`")
        ...
        await asyncio.sleep(2)
        await edit_or_reply(event, "**SAVING OUTPUT TO personalBot LOCAL DATABASE**")
        await asyncio.sleep(3.5)
        await edit_or_reply(event, 
            "Your[personalBot](https:/t.me/personalUserbot) is working Fine...\n       Join @personalSupport For Any Help......"
        )
